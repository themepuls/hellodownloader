...
function computeContentBox(
  ...
  // Partial OCR on dense text overlays — keep full width so left-side text is not cropped.
...
/** Fit entire thumbnail inside target ratio with blurred background — preserves text & faces. */
...
/** Crop anchored on OCR content box so text is not cut off when ratios are similar. */
...
/**
 * AI Adjust resize: preserve OCR text and subjects when converting aspect ratio.
 * Uses blur-padded contain for large ratio changes; content-anchored crop otherwise.
 */
